> **架构说明（2026-05-11 更新）：** 本文档中的角色采用"外部 CLI 引擎"模式。每个 Hermes Profile 是轻量编排层，实际工作委托给外部 CLI 引擎。详见 [EXTERNAL-CLI-ENGINE.md](./EXTERNAL-CLI-ENGINE.md)。
...
> **拦截层级说明：** Risk Policy Engine 在 Hermes Plugin 的 `pre_tool_call` 钩子层拦截——该钩子包裹了 `terminal()` 调用，而 `terminal()` 正是 Profile 调用外部 CLI 引擎的入口。因此，风险策略在命令实际送达 CLI 引擎之前即可阻断。
