> **架构说明（2026-05-11 更新）：** 本文档中的 AI 失败模式已区分两个层级：**Profile 层失败**（路由错误、协议解析）和 **CLI 引擎层失败**（推理错误、工具误用）。详见 [EXTERNAL-CLI-ENGINE.md](./EXTERNAL-CLI-ENGINE.md)。
...
### D.1 Implementer — 过度自信地自行决定 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确路由了任务，但 CLI 引擎（LLM）因 helpfulness bias 自行做出了超出权限的决策。
...
### D.2 Implementer — 未发现隐藏的安全问题 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了任务，但 CLI 引擎对库默认配置的知识缺失导致安全漏洞。
...
### D.3 Tech-Reviewer — 误报安全"问题" `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了审查任务，但 CLI 引擎对密码学库的误判导致误报。
...
### D.4 Tech-Reviewer — 遗漏真正的安全问题 `[Profile 层 + CLI 引擎层]`

> **失败层级：** 两层兼有。Profile 层的审查范围仅限 `changed_files`（设计局限），CLI 引擎层未主动扩大审查范围。
...
### D.5 PM — 任务拆分过细 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确路由了拆分请求，但 CLI 引擎对"原子任务"的粒度判断偏机械。

**【失败场景】**

PM 收到"实现 JWT 认证模块"的需求后，拆分了 15 个子任务：
- T1: 生成 RSA 密钥对
...
- PM 的 SOUL.md 要求"拆分后自检：如果两个任务必须在同一个 worktree 中顺序执行，考虑合并"

---

### D.6 PM — 依赖关系设错 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了任务创建，但 CLI 引擎对任务间逻辑依赖的推理错误。

**【失败场景】**

PM 创建任务时，将 T3（测试）设置为 T2（HTTP 接口）的 **child**，但 T4（代码审查）设置为 T1（JWT 核心）的 child。
...
- PM 的 SOUL.md 要求"设置 parents 后，反向验证：每个下游任务拿到上游的 handoff 后能否独立完成？"
- 引入"依赖图可视化"：Jacky 可在任务创建后查看依赖图，发现错误时手动调整

---

### D.7 QA-Tester — 测试覆盖不全 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了测试任务，但 CLI 引擎的测试场景枚举不完整（尤其安全攻击向量）。
...
### D.8 DevOps-Engineer — 环境变量遗漏 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了部署任务，但 CLI 引擎生成的脚本缺少防御性检查（`set -u`）。
...
### D.9 SRE-Observer — 根因归因错误 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了故障分析，但 CLI 引擎的"就近归因"倾向导致根因分析深度不足。
...
### D.10 SRE-Observer — 过度自信的结论 `[CLI 引擎层失败]`

> **失败层级：** CLI 引擎层。Profile 正确派发了分析任务，但 CLI 引擎在证据不充分时给出了高置信度结论。
