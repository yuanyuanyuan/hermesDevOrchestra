...
**Goal:** 以 `.planning/phases/19-hermes-workflow-design/` 非归档文档为来源，先把 “Hermes 调度层 + 外部 CLI 引擎执行层” 的 MVP 纵向切片落地：验证官方能力边界、落成 profile/override、多项目基础隔离、补齐外部 CLI 引擎协议、Kanban 路由、风险护栏、worker 生命周期与基础可观测性。

- [x] **Phase 20: Capability Verification & Boundary Lock** — 验证 Hermes 官方能力，锁定哪些设计点属于官方覆盖、哪些必须本地实现。
- [x] **Phase 21: Profiles, Overrides & Board Isolation** — 落成 8 个 active profiles、3 个 reserved profiles、项目级 override 和多项目基础隔离约定。
- [x] **Phase 22: External CLI Engine Protocol & Role Invocation** — 把 profile 角色正式切到外部 CLI 引擎统一模式，落成 engine 配置、请求/响应协议和失败归一化处理。
- [x] **Phase 23: Stateful Routing & Kanban Handoff** — 以无状态 CLI 调用为前提，将需求拆解、角色派发、block-resume 交接全部切到 Kanban 状态机。
- [x] **Phase 24: Risk Policy & Role Guardrails** — 在外部 CLI 引擎模式下落成 L1/L2/L3 风险策略、Reviewer/Orchestrator 边界与 Implementer block 契约。
- [x] **Phase 25: Worker Lifecycle, Observability & MVP Acceptance** — completed 2026-05-11
...
**Goal:** 为 PM、Researcher、Implementer、Reviewer、QA-Tester、DevOps 和 SRE-Observer 落成统一的外部 CLI 引擎调用契约，让 Hermes Profile 只保留调度与 authority。  
**Depends on:** Phase 21  
**Requirements:** ENG-01, ENG-02
**Success Criteria:**
  1. profile catalog 与项目级 override 能声明 `engine.cli/mode/flags/fallback`，且不与 Hermes 路由层 `model` 字段混淆。
  2. `hermes-role-engine/v1` request/response envelope 在仓库产物中落成，并至少覆盖 PM 与 Implementer/Reviewer 的调用链。
  3. CLI timeout、crash、parse-error、rate-limit 都被规整为可审计的 retry → block 语义，不把厂商特有行为泄漏到工作流主链路。
**Plans:** 1/1 plans complete

### Phase 23: Stateful Routing & Kanban Handoff
**Goal:** 在无状态 CLI 调用前提下，用 Kanban 状态机替换旧文件总线路由，打通 PM、Researcher、Orchestrator 和 Worker 的主链路交接。  
**Depends on:** Phase 22  
**Requirements:** ENG-03, ROUTE-01, ROUTE-02
**Success Criteria:**
  1. PM 可以把澄清结果或 research 输出转换成带 parents 的任务图并分配到目标 profile。
  2. Orchestrator 基于状态枚举、Kanban metadata 与路由表派发任务，不依赖旧文件总线或 CLI session resume。
  3. Worker 的澄清、阻塞、恢复和完成流全部通过 Kanban 原语与上下文累积字段完成。
**Plans:** 1/1 plans complete

### Phase 24: Risk Policy & Role Guardrails
**Goal:** 在外部 CLI 引擎模式下落成风险策略、只读护栏和 Implementer 的显式 block 契约，防止角色越权。  
**Depends on:** Phase 23  
**Requirements:** SAFE-01, SAFE-02, SAFE-03
**Success Criteria:**
  1. 风险策略 YAML 经 `pre_tool_call` hook 拦截后，L3 操作无法自动通过。
  2. Reviewer 与 Orchestrator 的 allowlist/tool guards 能阻止 shell 写操作、文件写操作或代码执行越权。
  3. Implementer 在架构不确定、外部依赖异常、风险策略拦截和关键测试失败时会显式 `kanban_block`。
**Plans:** 1/1 plans complete

### Phase 25: Worker Lifecycle, Observability & MVP Acceptance
**Goal:** 落成 worker timeout、dirty cleanup、structured handoff、基础背压控制、hook 级 trace、环境快照与 MVP 验收，为 v1.4 提供明确 handoff。  
**Depends on:** Phase 24  
**Requirements:** EXEC-01, EXEC-02, EXEC-03, FLOW-01, OBS-01, OBS-02
**Success Criteria:**
  1. worker task metadata 明确包含 `expected_duration_max`，超时后可触发回收和重派。
  2. crash、timeout 或 cancel 后，worktree 能恢复到任务开始前的干净状态，handoff metadata 有 schema 约束且按 untrusted input 消费。
  3. `post_tool_call` / `on_session_end` traces 与 spawn 时的环境快照已经落到可查询存储，不修改 Hermes 核心。
  4. 至少一个从需求 → 实现 → 审查/测试的 MVP 主链路通过验收，并给 v1.4 明确记录 curator 语义、SRE RCA 与 deploy/UAT 延后项。
**Plans:** 1/1 plans complete
...
| 22. External CLI Engine Protocol & Role Invocation | v1.3 | 1/1 | Complete | 2026-05-11 |
| 23. Stateful Routing & Kanban Handoff | v1.3 | 1/1 | Complete | 2026-05-11 |
| 24. Risk Policy & Role Guardrails | v1.3 | 1/1 | Complete | 2026-05-11 |
| 25. Worker Lifecycle, Observability & MVP Acceptance | v1.3 | 1/1 | Complete | 2026-05-11 |
