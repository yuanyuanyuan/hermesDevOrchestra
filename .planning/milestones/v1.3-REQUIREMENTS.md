# Requirements Archive: v1.3 Hermes 原生工作流 MVP 实现

**Archived:** 2026-05-11
**Status:** SHIPPED

Next milestone requirements will be created fresh with `$gsd-new-milestone`.
...
**Milestone:** v1.3 — Hermes 原生工作流 MVP 实现  
**Goal:** 基于 `.planning/phases/19-hermes-workflow-design/` 的非归档设计文档，先完成 “Hermes 调度层 + 外部 CLI 引擎执行层” 的 MVP 纵向切片：验证官方能力边界，落成 8 个 profile 与项目级隔离，补齐 CLI 引擎协议，打通 Kanban 路由、风险护栏、worker 生命周期和基础可观测性。
...
- [x] **PROF-01**: 用户可以在不污染 `~/.hermes/profiles/` 全局配置的前提下，为单个项目覆盖 profile 的 model、toolsets 和 SOUL 规则。
- [x] **PROF-02**: 系统提供 8 个启用中的 workflow profiles 和 3 个禁用的预留 profiles，每个 profile 都有明确的 toolset allowlist、SOUL 边界和职责说明。
- [x] **FLOW-02**: 用户可以并行运行多个 project boards，而不会出现 task、workspace、profile 或评论跨项目串线。
- [x] **MEM-01**: Learnings 默认写入项目命名空间；只有显式 cross-project 标记并经过编排层批准的条目才允许进入全局命名空间。

### 外部 CLI 引擎协议

- [x] **ENG-01**: 每个 workflow profile 都可以独立声明 Hermes 路由层 `model` 与外部 CLI `engine`（`cli/mode/flags/fallback`），并允许项目级 override 在不污染全局 profile 的前提下切换底层执行引擎。
- [x] **ENG-02**: Hermes Profile 与外部 CLI 引擎之间统一使用 `hermes-role-engine/v1` JSON request/response envelope；CLI timeout、crash、parse-error 和 rate-limit 都按可审计的 retry → block 策略处理。
- [x] **ENG-03**: PM 澄清、多轮 research 衔接和 worker 恢复必须依赖 Kanban/task metadata 的上下文累积，而不是 CLI session resume；Orchestrator 保持规则驱动，不使用外部 CLI 引擎做路由决策。

### 路由与任务流转

- [x] **ROUTE-01**: PM 可以把需求澄清结果转换成带依赖的 Kanban 任务图，由 Orchestrator 按状态机把任务派给 Researcher、Implementer、Reviewer 和 QA-Tester。
- [x] **ROUTE-02**: Worker 在遇到澄清、审查或恢复场景时，可以通过 `kanban_block` / `kanban_comment` / `kanban_complete` 完成 block-resume 交接，不再依赖旧文件总线。

### 风险与角色护栏

- [x] **SAFE-01**: Risk policy YAML 通过 Hermes `pre_tool_call` hook 将命令分级为 L1/L2/L3，并保证 L3 操作永不自动批准。
- [x] **SAFE-02**: Reviewer 与 Orchestrator 采用 allowlist toolsets 和只读 terminal guardrails，不能通过 shell 或文件写操作绕过角色边界。
- [x] **SAFE-03**: Implementer 的 SOUL/skills 强制在架构决策、外部依赖不可用、风险策略拦截和关键测试失败时触发 `kanban_block`。

### Worker 生命周期与交接

- [x] **EXEC-01**: 每个 worker task 都声明 `expected_duration_max`，dispatcher 能在超时后执行回收并把任务重派。
- [x] **EXEC-02**: worker crash、timeout 或 cancel 后，系统会把该任务占用的 worktree 恢复到开始前的干净状态。
- [x] **EXEC-03**: `kanban_complete` 输出结构化 handoff metadata，且下游 worker 只能把这些 metadata 当作 untrusted input 使用。
- [x] **FLOW-01**: Dispatcher 能根据按 profile 分类的 ready 队列深度进行基础限流或暂停，避免上游 worker 把下游审查/测试队列压垮。

### 基础可观测性

- [x] **OBS-01**: Observability plugin 通过 Hermes 官方 hooks 采集 `post_tool_call` 和 `on_session_end` 事件，不修改 Hermes 核心代码。
- [x] **OBS-02**: Dispatcher 在 spawn worker 时附带环境快照（至少包含 `git status`、`df -h` 前 5 行、`hermes status`）到 task run metadata，供后续调试使用。
...
| Feature | Reason |
|---------|--------|
| v1.4 的 curator 语义合并、SRE RCA 自动化和三层部署闭环 | 本次只创建基于外部 CLI 引擎的 v1.3 MVP，完整能力已拆分到下一 milestone。 |
...
| ROUTE-01 | Phase 23 | Complete |
| ROUTE-02 | Phase 23 | Complete |
| SAFE-01 | Phase 24 | Complete |
| SAFE-02 | Phase 24 | Complete |
| SAFE-03 | Phase 24 | Complete |
| EXEC-01 | Phase 25 | Complete |
| EXEC-02 | Phase 25 | Complete |
| EXEC-03 | Phase 25 | Complete |
| FLOW-01 | Phase 25 | Complete |
| OBS-01 | Phase 25 | Complete |
...
*Last updated: 2026-05-11 — v1.3 execution complete through Phase 25*
