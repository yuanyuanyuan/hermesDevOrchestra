...
def detect_cicd_config(files_changed: list[str], file_contents: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    # Check file paths for test/lint/deploy conventions (filename prefix
    # `test_` / directory `tests/` / `lint_` / `deploy_` etc.) — the path
    # convention has its own rules because filename underscores are valid
    # word characters and a pure word-boundary regex would miss real test
    # files like `test_module.py`.
    import re

    test_path_pattern = re.compile(r"(?:^|/)tests?(?:/|$)|(?:^|/)test_", re.IGNORECASE)
    lint_path_pattern = re.compile(r"(?:^|/)lint(?:/|$)|(?:^|/)lint_", re.IGNORECASE)
    deploy_path_pattern = re.compile(r"(?:^|/)deploy(?:ment|s|ed)?(?:/|$)|(?:^|/)deploy_", re.IGNORECASE)
    for filepath in files_changed:
        if test_path_pattern.search(filepath):
            cicd_config["has_tests"] = True
        if lint_path_pattern.search(filepath):
            cicd_config["has_linting"] = True
        if deploy_path_pattern.search(filepath):
            cicd_config["has_deployment"] = True

    # Check file contents for CI/CD indicators using whole-word matching to
    # avoid false positives from substrings (e.g. "attestation", "latest",
    # "contest"). Content is the place where word boundaries actually
    # matter; for paths we rely on the convention matcher above.
    if file_contents:
        ...
        for filepath, content in file_contents.items():
            ...
            if test_pattern.search(content_lower):
                cicd_config["has_tests"] = True
            if lint_pattern.search(content_lower):
                cicd_config["has_linting"] = True
            if deploy_pattern.search(content_lower):
                cicd_config["has_deployment"] = True
