...
TEST_NAME="gateway-worker-output-identity-mismatch"
...
PROJECT_ID="gateway-worker-identity"
...
payload = {
    "idempotency_key": "gw-021-create",
    "ticket": {
        "background": "Advancement Gate identity test",
        "goal": "Reject worker output for the wrong task correlation",
        "deliverables": ["Identity mismatch evidence"],
        "acceptance_criteria": ["Wrong correlation cannot complete a task"],
        "hard_constraints": ["Only matched task identity may advance"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block on identity mismatch"
    },
...
payload = {
    "idempotency_key": "gw-021-worker-output",
    "task_id": task_id,
    "worker_response": {
        "protocol": "hermes-role-engine/v1",
        "role": "implementer",
        "correlation_id": f"{task_id}-wrong",
        "turn": 1,
        "status": "completed",
...
assert output["failure_class"] == "identity_mismatch", output
...
assert status["blocked_reason"] == "worker_output_identity_mismatch", status

task = next(item for item in tasks["tasks"] if item["task_id"] == task_id)
assert task["status"] == "blocked", tasks
assert task["blocked_reason"] == "worker_output_identity_mismatch", task
...
assert report["failure_class"] == "identity_mismatch", report
assert "correlation_id" in report["violations"], report
...
assert any(record.get("type") == "worker_output_blocked" and record.get("failure_class") == "identity_mismatch" for record in audit_records), audit_records
