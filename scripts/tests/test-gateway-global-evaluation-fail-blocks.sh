...
TEST_NAME="gateway-global-evaluation-fail-blocks"
...
PROJECT_ID="gateway-global-fail"
...
payload = {
    "idempotency_key": "gw-041-create",
    "ticket": {
        "background": "Global evaluation fail test",
        "goal": "Block Stage 5 failure instead of entering closeout",
        "deliverables": ["Failure decision evidence"],
        "acceptance_criteria": ["fail verdict does not enter Stage 6"],
        "hard_constraints": ["Failures cannot be hidden by closeout"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block for Kimi decision when repair cannot proceed automatically"
    },
...
assert status["blocked_reason"] == "global_evaluation_failed", status
...
assert any(record.get("type") == "global_evaluation_recorded" and record.get("decision") == "FAIL" and record.get("failure_class") == "global_evaluation_failed" for record in audit_records), audit_records
