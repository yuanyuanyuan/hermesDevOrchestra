...
"$REPO_ROOT/scripts/bin/orch-approve" "$approval_id" "approved fixture" >/tmp/orch-risk-approved.out
"$REPO_ROOT/scripts/bin/orch-bus-loop" test-proj "$PROJECT_DIR" --once
...
set +e
"$REPO_ROOT/scripts/bin/orch-approve" "$l4_approval_id" "approved fixture" >/tmp/orch-risk-l4-bad.out 2>/tmp/orch-risk-l4-bad.err
bad_phrase_status=$?
set -e
assert_exit_code 7 "$bad_phrase_status" "L4 approval should require fixed phrase"
"$REPO_ROOT/scripts/bin/orch-approve" "$l4_approval_id" "APPROVE-L4 $l4_approval_id" >/tmp/orch-risk-l4-good.out
"$REPO_ROOT/scripts/bin/orch-bus-loop" test-proj "$PROJECT_DIR" --once
