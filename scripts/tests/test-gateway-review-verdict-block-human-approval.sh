...
TEST_NAME="gateway-review-verdict-block-human-approval"
...
PROJECT_ID="gateway-review-human"
...
payload = {
    "idempotency_key": "gw-027-create",
    "ticket": {
        "background": "Human approval verdict routing test",
        "goal": "Block on high-risk review verdict",
        "deliverables": ["Decision required evidence"],
        "acceptance_criteria": ["Human approval block remains active and recoverable"],
        "hard_constraints": ["Kimi must not override Human Approval boundary"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block and require human decision"
    },
...
payload = {
    "idempotency_key": "gw-027-verdict",
    "task_id": task_id,
    "verdict": {
        ...
        "review_kind": "security_review",
        "verdict": "block",
        "findings": [
            {
                "finding_id": "SEC-001",
                "severity": "critical",
                "summary": "Review found a permission boundary change",
                "affected_files": ["config/risk-policy.yaml"],
                "affected_acceptance_criteria_refs": [],
                "evidence_refs": [f"state://runs/{run_id}/run.json"],
                "required_fix": "Get explicit Human Approval before continuing"
            }
        ],
        "affected_acceptance_criteria_refs": [],
        "required_fixes": ["Get explicit Human Approval before continuing"],
        "evidence_refs": [f"state://runs/{run_id}/run.json"],
...
assert status["blocked_reason"] == "review_blocked_human_approval_required", status
...
assert task["blocked_reason"] == "review_blocked_human_approval_required", task
...
assert any(record.get("type") == "review_verdict_recorded" and record.get("decision") == "BLOCKED" for record in audit_records), audit_records
