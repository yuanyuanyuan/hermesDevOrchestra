...
TEST_NAME="gateway-global-evaluation-warnings"
...
PROJECT_ID="gateway-global-warnings"
...
payload = {
    "idempotency_key": "gw-029-create",
    "ticket": {
        "background": "Global evaluation warning test",
        "goal": "Require final acceptance for warnings",
        "deliverables": ["Final acceptance decision blocker"],
        "acceptance_criteria": ["pass_with_warnings does not enter Stage 6 automatically"],
        "hard_constraints": ["Warnings require explicit Kimi acceptance below human-risk gates"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block for final acceptance"
    },
...
assert response["route_result"] == "final_acceptance_required", response
...
assert status["blocked_reason"] == "global_evaluation_acceptance_required", status
...
assert any(record.get("type") == "global_evaluation_recorded" and record.get("command_id") == response["command_id"] for record in audit_records), audit_records
