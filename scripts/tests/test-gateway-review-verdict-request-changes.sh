...
TEST_NAME="gateway-review-verdict-request-changes"
...
PROJECT_ID="gateway-review-verdict"
...
payload = {
    "idempotency_key": "gw-026-create",
    "ticket": {
        "background": "Review verdict routing test",
        "goal": "Route review changes into bounded improvement",
        "deliverables": ["Improvement stage queued from review feedback"],
        "acceptance_criteria": ["A request_changes review verdict does not complete the run"],
        "hard_constraints": ["Review artifacts are immutable"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Route recoverable review feedback into improvement"
    },
...
payload = {
    "idempotency_key": "gw-026-verdict",
    "task_id": task_id,
    "verdict": {
        ...
        "findings": [
            {
                "finding_id": "F-001",
                "severity": "medium",
                "summary": "Implementation drifted from acceptance criteria",
                "affected_files": ["scripts/example.py"],
                "affected_acceptance_criteria_refs": [],
                "evidence_refs": [f"state://runs/{run_id}/run.json"],
                "required_fix": "Align implementation with accepted scope"
            }
        ],
        "affected_acceptance_criteria_refs": [],
        "required_fixes": ["Align implementation with accepted scope"],
        "evidence_refs": [f"state://runs/{run_id}/run.json"],
...
assert response["route_result"] == "improvement_queued", response
assert response["verdict_ref"].startswith(f"state://runs/{run_id}/review-verdicts/"), response
assert response["improvement_cycle"] == 1, response

assert status["status"] == "queued", status
assert status["current_stage"] == "improvement", status
...
implementation_task = next(item for item in tasks["tasks"] if item["task_id"] == task_id)
assert implementation_task["status"] == "blocked", implementation_task
assert implementation_task["blocked_reason"] == "review_changes_requested", implementation_task
assert response["verdict_ref"] in implementation_task["artifact_refs"], implementation_task
improvement_task = next(item for item in tasks["tasks"] if item["stage"] == "improvement")
assert improvement_task["status"] == "queued", improvement_task
...
assert verdict["artifact_type"] == "review_report", verdict
assert verdict["verdict"] == "request_changes", verdict
assert verdict["within_approved_scope"] is True, verdict
...
assert any(record.get("type") == "review_verdict_recorded" and record.get("command_id") == response["command_id"] for record in audit_records), audit_records
