...
TEST_NAME="gateway-qa-verdict-block-kimi"
...
PROJECT_ID="gateway-qa-block-kimi"
...
payload = {
    "idempotency_key": "gw-044-create",
    "ticket": {
        "background": "QA block test",
        "goal": "Route QA block verdict to a Kimi decision",
        "deliverables": ["QA block evidence"],
        "acceptance_criteria": ["QA block does not enter closeout"],
        "hard_constraints": ["QA failures require explicit decision"],
        "soft_constraints": [],
        "related_tasks": [],
...
verdict = {
    ...
    "review_kind": "qa",
    "verdict": "block",
    "findings": [{"finding_id": "QA-001", "summary": "Required browser QA evidence is missing"}],
    "affected_acceptance_criteria_refs": ["AC-001"],
    "required_fixes": ["Provide executable QA evidence"],
...
payload = {
    "idempotency_key": "gw-044-verdict",
    "task_id": task_id,
    "verdict": verdict
}
request = urllib.request.Request(
...
assert response["failure_class"] == "qa_blocked", response
...
assert status["blocked_reason"] == "qa_blocked", status
...
assert task["blocked_reason"] == "qa_blocked", task
...
assert verdict["artifact_type"] == "qa_report", verdict
...
assert any(record.get("type") == "review_verdict_recorded" and record.get("failure_class") == "qa_blocked" for record in audit_records), audit_records
assert any(record.get("type") == "decision_required" and record.get("approval_id") == response["decision_id"] and record.get("authority_required") == "kimi" for record in audit_records), audit_records

calls = pathlib.Path(hermes_log).read_text(encoding="utf-8").splitlines()
assert any("kanban block" in line and "qa_blocked" in line for line in calls), calls
PY
