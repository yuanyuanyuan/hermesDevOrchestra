...
TEST_NAME="gateway-global-evaluation-artifact-ref-scope"
...
PROJECT_ID="gateway-global-artifact-scope"
...
payload = {
    "idempotency_key": "gw-039-create",
    "ticket": {
        "background": "Global evaluation artifact ref scope test",
        "goal": "Reject absolute artifact refs in global evaluation scalar fields",
        "deliverables": ["Scoped ref validation"],
        "acceptance_criteria": ["Absolute development_plan_ref is rejected"],
        "hard_constraints": ["Artifact refs stay scoped to the current run"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Reject invalid report before writing authority artifacts"
    },
...
report = {
    ...
    "development_plan_ref": "/tmp/leaked-development-plan.json",
    "debate_report_refs": [],
...
payload = {"idempotency_key": "gw-039-global-evaluation", "report": report}
request = urllib.request.Request(
...
assert "development_plan_ref" in response["violations"], response

assert status["status"] == "queued", status
assert "global_evaluation_report" not in status.get("artifact_refs", {}), status
...
assert not any(record.get("type") == "global_evaluation_recorded" for record in audit_records), audit_records
PY
