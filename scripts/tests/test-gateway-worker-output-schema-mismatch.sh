...
TEST_NAME="gateway-worker-output-schema-mismatch"
...
PROJECT_ID="gateway-worker-schema"
...
payload = {
    "idempotency_key": "gw-020-create",
    "ticket": {
        "background": "Advancement Gate test",
        "goal": "Reject malformed worker completion",
        "deliverables": ["Blocked task evidence"],
        "acceptance_criteria": ["Schema mismatch cannot complete a task"],
        "hard_constraints": ["Only Gateway may advance lifecycle"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block on schema mismatch"
    },
...
payload = {
    "idempotency_key": "gw-020-worker-output",
    "task_id": task_id,
    "worker_response": {
        "protocol": "not-hermes-role-engine/v1",
        "role": "implementer",
...
assert output["schema_version"] == "orchestra.v1", output
assert output["run_id"] == run_id, output
assert output["task_id"] == task_id, output
assert output["gate_result"] == "blocked", output
assert output["failure_class"] == "schema_mismatch", output
assert output["validation_report_ref"].startswith(f"state://runs/{run_id}/worker-output-validation-reports/"), output
assert output["event_projection_degraded"] is False, output

assert status["status"] == "blocked", status
assert status["blocked_reason"] == "worker_output_schema_mismatch", status
assert output["validation_report_ref"] in status["pending_decision_refs"], status

task = next(item for item in tasks["tasks"] if item["task_id"] == task_id)
assert task["status"] == "blocked", tasks
assert output["validation_report_ref"] in task["artifact_refs"], task

event_types = [event["type"] for event in events["events"]]
assert "worker_output_blocked" in event_types, event_types
assert "task_completed" not in event_types, event_types
blocked_event = next(event for event in events["events"] if event["type"] == "worker_output_blocked")
assert blocked_event["status"] == "blocked", blocked_event
assert blocked_event["task_id"] == task_id, blocked_event
assert blocked_event["artifact_refs"] == [output["validation_report_ref"], f"state://runs/{run_id}/run.json"], blocked_event
...
assert report["artifact_type"] == "worker_output_validation_report", report
assert report["result"] == "blocked", report
assert report["failure_class"] == "schema_mismatch", report
assert "protocol" in report["violations"], report
...
assert any(record.get("type") == "worker_output_blocked" and record.get("command_id") == output["command_id"] for record in audit_records), audit_records
