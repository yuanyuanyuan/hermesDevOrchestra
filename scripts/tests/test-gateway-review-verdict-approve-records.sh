...
TEST_NAME="gateway-review-verdict-approve-records"
...
PROJECT_ID="gateway-review-approve"
...
payload = {
    "idempotency_key": "gw-045-create",
    "ticket": {
        "background": "Review approve test",
        "goal": "Record approved review verdict as evidence",
        "deliverables": ["Approval verdict evidence"],
        "acceptance_criteria": ["approve verdict does not block or complete the run by itself"],
        "hard_constraints": ["Approval text is evidence, not completion authority"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Keep workflow authority separate from review evidence"
    },
...
payload = {
    "idempotency_key": "gw-045-verdict",
    "task_id": task_id,
    "verdict": verdict
}
request = urllib.request.Request(
...
assert response["route_result"] == "approved", response
...
assert "failure_class" not in response, response
...
assert "blocked_reason" not in task, task
...
assert verdict["command_id"] == response["command_id"], verdict
...
assert any(record.get("type") == "review_verdict_recorded" and record.get("decision") == "APPROVED" for record in audit_records), audit_records
...
assert not any("kanban block" in line or "kanban complete" in line for line in calls), calls
PY
