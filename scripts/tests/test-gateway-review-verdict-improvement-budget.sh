...
TEST_NAME="gateway-review-verdict-improvement-budget"
...
PROJECT_ID="gateway-review-budget"
...
payload = {
    "idempotency_key": "gw-028-create",
    "ticket": {
        "background": "Stage 4 budget test",
        "goal": "Prevent unbounded improvement loops",
        "deliverables": ["Improvement exhaustion blocker"],
        "acceptance_criteria": ["Second request_changes blocks instead of starting another automatic improvement"],
        "hard_constraints": ["Only one automatic improvement cycle in MVP"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block after one failed improvement cycle"
    },
...
python3 - "$BASE_URL" "$RUN_ID" "$TASK_ID" "$TMP_DIR/first-verdict.json" <<'PY'
import json
import sys
import urllib.request
...
payload = {
    "idempotency_key": "gw-028-verdict-1",
    "task_id": task_id,
    "verdict": {
        ...
        "required_fixes": ["Fix the implementation drift"],
...
FIRST_VERDICT_REF="$(python3 - "$TMP_DIR/first-verdict.json" <<'PY'
import json
import sys
print(json.load(open(sys.argv[1], encoding="utf-8"))["verdict_ref"])
PY
)"

python3 - "$BASE_URL" "$RUN_ID" "$TASK_ID" "$FIRST_VERDICT_REF" "$TMP_DIR/second-verdict.json" <<'PY'
import json
import sys
import urllib.request

base_url, run_id, task_id, first_verdict_ref, response_path = sys.argv[1:]
payload = {
    "idempotency_key": "gw-028-verdict-2",
    "task_id": task_id,
    "verdict": {
        "schema_version": "orchestra.v1",
        ...
        "findings": [],
        "affected_acceptance_criteria_refs": [],
        "required_fixes": ["Fixes still do not satisfy review"],
        "evidence_refs": [f"state://runs/{run_id}/run.json", first_verdict_ref],
        "within_approved_scope": True,
        "risk_level": "medium",
        "authority_required": "kimi",
        "improvement_cycle": 1,
...
assert response["failure_class"] == "improvement_exhausted", response
assert response["authority_required"] == "kimi", response

assert status["status"] == "blocked", status
assert status["blocked_reason"] == "improvement_exhausted", status
...
assert task["blocked_reason"] == "improvement_exhausted", task
...
assert any(record.get("type") == "review_verdict_recorded" and record.get("failure_class") == "improvement_exhausted" for record in audit_records), audit_records
