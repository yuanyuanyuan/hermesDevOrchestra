...
"$REPO_ROOT/scripts/bin/orch-decisions" test-proj > /tmp/orch-decision-cli-list.out
...
"$REPO_ROOT/scripts/bin/orch-approve" "$id_approve" "APPROVED fixture" >/tmp/orch-decision-cli-approve.out
assert_contains "APPROVED $id_approve" /tmp/orch-decision-cli-approve.out "approval output missing"
"$REPO_ROOT/scripts/bin/orch-reject" "$id_reject" "REJECTED fixture" >/tmp/orch-decision-cli-reject.out
assert_contains "REJECTED $id_reject" /tmp/orch-decision-cli-reject.out "rejection output missing"

set +e
"$REPO_ROOT/scripts/bin/orch-decisions" "../outside" >/tmp/orch-decision-cli-traversal.out 2>&1
decisions_traversal=$?
"$REPO_ROOT/scripts/bin/orch-audit" "../outside" >/tmp/orch-audit-traversal.out 2>&1
audit_traversal=$?
set -e
