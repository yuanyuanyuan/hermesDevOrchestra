...
TEST_NAME="gateway-worker-output-evidence-missing"
...
PROJECT_ID="gateway-worker-evidence"
...
payload = {
    "idempotency_key": "gw-024-create",
    "ticket": {
        "background": "Advancement Gate evidence test",
        "goal": "Reject code-changing completion without tests",
        "deliverables": ["Evidence-missing validation report"],
        "acceptance_criteria": ["Completion requires test evidence"],
        "hard_constraints": ["Worker summary alone is not authority"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block missing evidence"
    },
...
payload = {
    "idempotency_key": "gw-024-worker-output",
    "task_id": task_id,
    "worker_response": {
...
assert output["failure_class"] == "evidence_missing", output

assert status["status"] == "blocked", status
assert status["blocked_reason"] == "worker_output_evidence_missing", status

task = next(item for item in tasks["tasks"] if item["task_id"] == task_id)
assert task["status"] == "blocked", tasks
assert task["blocked_reason"] == "worker_output_evidence_missing", task
...
assert report["failure_class"] == "evidence_missing", report
assert "test_evidence_refs" in report["violations"], report
...
assert any(record.get("type") == "worker_output_blocked" and record.get("failure_class") == "evidence_missing" for record in audit_records), audit_records
