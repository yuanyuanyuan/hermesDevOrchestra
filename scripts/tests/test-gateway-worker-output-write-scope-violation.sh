...
TEST_NAME="gateway-worker-output-write-scope-violation"
...
PROJECT_ID="gateway-worker-scope"
...
payload = {
    "idempotency_key": "gw-023-create",
    "ticket": {
        "background": "Advancement Gate write scope test",
        "goal": "Reject worker output that touches forbidden targets",
        "deliverables": ["Write scope violation evidence"],
        "acceptance_criteria": ["Forbidden target cannot complete a task"],
        "hard_constraints": ["Root rule files require approval"],
        "soft_constraints": [],
        "related_tasks": [],
        "failure_strategy": "Block write-scope violations"
    },
...
payload = {
    "idempotency_key": "gw-023-worker-output",
    "task_id": task_id,
    "worker_response": {
        ...
        "role_specific_payload": {
            ...
            "diff_summary": "Touched a root rule file",
            "write_scope_result": {
                "within_scope": False,
                "violations": ["AGENTS.md is outside the approved write scope"],
                "forbidden_paths_touched": ["AGENTS.md"]
            },
            "test_evidence_refs": [],
            "risk_notes": ["root rule change requires approval"],
            "approval_refs": []
        },
...
assert output["failure_class"] == "write_scope_violation", output

assert status["status"] == "blocked", status
assert status["blocked_reason"] == "worker_output_write_scope_violation", status

task = next(item for item in tasks["tasks"] if item["task_id"] == task_id)
assert task["status"] == "blocked", tasks
assert task["blocked_reason"] == "worker_output_write_scope_violation", task
...
assert report["failure_class"] == "write_scope_violation", report
assert "write_scope_result" in report["violations"], report
...
assert any(record.get("type") == "worker_output_blocked" and record.get("failure_class") == "write_scope_violation" for record in audit_records), audit_records
