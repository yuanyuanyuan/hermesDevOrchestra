...
| Strict 0→6 staging regression | `scripts/lib/staging Harness.sh`, `scripts/lib/staging inject-data.sh`, `scripts/tests/test-e2e-strict-six-stage-flow.sh` | ready | test harness | Replays intake, direction, solution, implementation, improvement, global evaluation, and closeout in `.hermes/staging/`, then verifies artifacts and metrics gate output. ⚠ File names contain spaces (`staging Harness.sh` / `staging inject-data.sh`); markdown backtick links are not clickable — invoke from shell directly. |
