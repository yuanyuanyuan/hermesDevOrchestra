...
| Gateway full runtime implementation | `scripts/lib/orch_gateway.py` plus future full runtime work | pending | MVP/current runtime active | Current executable Gateway remains MVP/current runtime until artifact-family readiness gates activate full contracts. |
...
| Full debate team registry | `config/debate/full/teams.json` | staged | not active runtime | Sixteen qnN4o510 canonical teams with at least three members each. |
| Full debate mode registry | `config/debate/full/modes.json` | staged | not active runtime | Eight qnN4o510 canonical debate modes. |
| Debate coverage policy | `config/debate/full/coverage-policy.json` | staged | not active runtime | Required stage coverage and partial-report policy. |
| Debate assembly policy | `config/debate/full/assembly-policy.json` | staged | not active runtime | Deterministic stage, task-type, risk, override, and member-scoring selector. |
| Debate backend policy | `config/debate/full/backend-policy.json` | staged | not active runtime | Template fallback is degraded fixture only. |
| Current debate runtime config | `config/debate/teams.json`, `config/debate/modes.json` | ready | active/current | Legacy MVP runtime registries; not full package authority. |
...
| Runtime Domain Knowledge Base config | `config/knowledge/runtime-kb.json` | disabled | not implemented | gbrain-backed target, no separate Hermes SQLite KB. |
| Runtime knowledge entry contract | Full schema + `config/knowledge/runtime-kb.json` | ready | not implemented | gbrain markdown page with YAML frontmatter and required sections. |
| Runtime knowledge ingestion audit | Full schema `knowledge_ingestion_record` | ready | not implemented | Promotion, overwrite, supersession, deprecation, and failed re-verification require records. |
| Runtime knowledge retrieval audit | Full schema `runtime_knowledge_query/result` | ready | not implemented | Retrieval results are context, not final authority. |
| Full worker backend registry | `config/workers/full/backends.json` | staged | not active runtime | Explicit backend capabilities, checks, workspace/session support, risk ceiling, and fallback eligibility. |
| Full worker role registry | `config/workers/full/roles.json` | staged | not active runtime | Required capabilities, preferred backend, explicit fallbacks, allowed failure classes, and fallback-forbidden conditions. |
| Capability negotiation report | Full schema `capability_negotiation_report` | ready | not implemented | Blocks silent backend substitution and records unavailable or incompatible backend evidence. |
| Worker session lifecycle | Full schema `worker_session_record` | ready | not implemented | Task-scoped ephemeral tmux session record. |
| Worker parallel integration | Full schema `parallel_group_plan`, `conflict_scan`, `merge_conflict_report` | ready | not implemented | Requires isolated workspaces and serial integration. |
