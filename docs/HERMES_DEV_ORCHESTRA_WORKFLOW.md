...
> 前提：需求已定好，有 prd.md（无 PRD 时走降级路径，见阶段 0）
...
| **代码级跨 AI Review** | **Codex** | **`$gsd-code-review N --depth quick`** | 非交互 |
...
2. 判断是否有 PRD 文件？
   → YES: 派发 $gsd-new-project --auto @docs/prd.md 给 Codex
   → NO（仅有自然语言描述）:
     a. 将用户描述写入 .planning/brief.md
     b. 派发 /gsd-new-project（交互式）给 Claude，附带 brief.md
     c. Claude 生成 PRD 后继续后续流程
...
Step 7: 代码审查（Claude Code）
    ...
    产出: REVIEW.md
    ↓
Step 8: 代码级跨 AI Review — Codex 审查代码
    ↓
    $gsd-code-review N --depth quick
    ↓
    更新: REVIEW.md（Codex 视角的代码审查）
    ↓
...
def read_gsd_state(project_dir: str) -> dict:
    ...
    注意：依赖 GSD 原生 STATE.md 的输出格式。
    如果 GSD 版本变更导致格式变化，此处需要同步更新。
    解析失败时返回 status: "parse_error"，由调用方决定降级策略。
    """
    import re
    ...
    if match:
        ...
        # 无法解析 active_phase → 标记为 parse_error
        # 调用方应对 execute/verify/ship 等不可逆操作 fail closed
        state["status"] = "parse_error"
        state["error"] = "Could not parse active_phase from STATE.md"
...
# ─── Fail-Closed 规则 ───
# 对 execute/verify/ship 等不可逆操作：
# - 如果 read_gsd_state() 返回 status == "parse_error"，必须暂停并要求用户确认
# - 如果 read_gsd_roadmap() 返回空列表，必须暂停并要求用户确认
# - 禁止使用"最新目录"作为 phase 回退策略（backlog phase 使用 999.x 命名）
# - 歧义时 fail closed，不要猜测继续
...
def read_gsd_roadmap(project_dir: str) -> list:
    ...
    if not phases:
        # ROADMAP.md 解析不出任何 phase → 可能是格式变更
        # 返回空列表，调用方应 fail closed
        pass
...
# - Codex: $gsd-code-review 2 --depth quick（代码级跨 AI Review）
...
10. **STATE.md 解析依赖**：`read_gsd_state()` 依赖 GSD 原生 STATE.md 的输出格式，GSD 版本升级可能导致解析失败；已添加 parse_error 检测和 fail-closed 规则
