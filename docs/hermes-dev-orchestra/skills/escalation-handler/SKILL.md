...
- 评估升级请求的风险等级
- 向用户（通过 CLI clarify 或 Telegram/Discord）请求最终决策
- 记录所有升级和决策的审计日志
- 执行用户批准后的操作
- 拒绝执行用户未批准的危险操作
...
$(date -Iseconds) | APPROVED | {project} | {escalation_type} | User: {user_decision}
Details: {escalation_content}
---
EOF")
...
# 1. 记录审计日志
terminal(command="cat >> /tmp/hermes-orchestra/audit.log << 'EOF'
$(date -Iseconds) | REJECTED | {project} | {escalation_type} | User: {user_decision}
Details: {escalation_content}
---
EOF")
...
- 如果用户通过 Telegram 回复但 Hermes 处于 SSH CLI 模式，需要确保消息路由正确
...
```bash
# 1. 检查审计日志存在
terminal(command="ls -la /tmp/hermes-orchestra/audit.log")

# 2. 测试写入权限
terminal(command="echo '$(date -Iseconds) | TEST | test-project | TEST | User: test' >> /tmp/hermes-orchestra/audit.log")
