...
- [ ] helper module 之间单向依赖，无循环引用（通过 `scripts/lib/import-cycle-check.sh` 验证）
- [ ] 探测报告生成不直接修改 `orch_gateway.py`

## 文档交付物
- [ ] `docs/CONFIGURATION.md` 更新：Gateway helper modules 注册方式
- [ ] `docs/CONFIGURATION.md` 更新：project-profile.yaml 格式说明与 project.json 迁移指南
- [ ] `docs/user-flow-guide_by_kimi.md` 更新：0 阶 Gateway 数据流 + 5 分钟 SLA 降级路径
- [ ] `docs/adr/0010-gateway-seam-extraction.md` 新建：seam 拆分决策与 fallback 策略

## 任务完成状态
- [ ] U1a — Gateway Seam Extraction & Intake Helper Modules
- [ ] U1b — Project Discovery Pipeline & Profile Generation
