...
## 架构红线合规
- [ ] 新增 `scripts/lib/actor_auth.py` 独立模块，Actor 认证逻辑未直接追加到 `orch_gateway.py`
- [ ] 新增 `scripts/lib/run_projection.py` 独立模块，Projection 聚合逻辑未直接追加到 `orch_gateway.py`
- [ ] `orch_gateway.py` 净增长 ≤ 60 行
- [ ] `actor-secrets.json` 与 `authority-matrix.json` 为分文件存储，`grep -i secret config/decisions/authority-matrix.json` 返回空
- [ ] `actor-secrets.json` 文件权限为 600（`stat -c %a config/decisions/actor-secrets.json` 输出 600）

## 文档交付物
- [ ] `docs/gateway-integration-architecture.md` 已更新，包含 Projection API 端点定义、Actor 认证时序图、权限矩阵 8 类能力映射表
- [ ] `docs/FULL-COVERAGE-MATRIX.md` 已标记 PRD §2.2 的 8 类权限为「已落地」
- [ ] `docs/FULL-CAPABILITY-AUTHORITY-MATRIX.md` 已补充 L3/L4 审批者 token 结构与 capability route 说明
- [ ] `docs/sprints/prd-by-kimi-user-flow-strict/schema.md` 已补充 `actor_token`、`projection_response`、`authority_matrix_view` 数据模型
- [ ] `config/decisions/authority-matrix.json` 已落地并可被 Gateway 加载

## 任务完成状态
- [ ] U7 — Gateway Run Projection API 与六类状态投影（所有 AC 断言通过）
