...
## 架构红线合规
- [x] 新增 `scripts/lib/write_scope_validator.py` 独立模块，write scope 校验未直接追加到 `orch_gateway.py`
- [x] 新增 `scripts/lib/evidence_gate.py` 独立模块，evidence gate 未直接追加到 `orch_gateway.py`
- [x] `orch_gateway.py` 净增长 ≤ 60 行
- [x] `worker_session_record` 的读写优先通过 `scripts/lib/worker_session.py` 完成，Gateway 未直接操作 `worker-sessions/*.json`
- [x] `config/decisions/authority-matrix.json` 中 `mutate_kanban_raw_state` 对 Kimi/Claude/Codex 均为 `allowed=false`

## 文档交付物
- [x] `docs/gateway-integration-architecture.md` 已更新，包含 dispatch 时序图、write scope 校验流程图、evidence gate 判定树
- [x] `docs/sprints/prd-by-kimi-user-flow-strict/schema.md` 已补充 `dispatch_token`、`worker_session_record`、`write_scope_check`、`evidence_gate_result` 数据模型
- [x] `docs/user-flow-guide_by_kimi.md` 已补充三阶分派的交互说明与错误码释义
- [x] `audit.jsonl` 中存在 `write_scope_check` 与 `source_isolation_check` 两类事件记录

## 任务完成状态
- [x] U8 — 三阶分派执行、工作区隔离与证据门控（所有 AC 断言通过）
...
[2026-06-03] 已合并到 main — PR #26 (`feat/sprint8`), merged 2026-06-02T07:46:44Z
