...
## 架构红线合规
- [x] 所有新增脚本使用 `set -euo pipefail`，错误即停
- [x] Staging 环境定义文件受版本控制，搭建脚本幂等
- [x] Schema 三重一致性校验零人工介入，纯自动化运行
- [x] 文件态持久化采用原子写模式

## 文档交付物
- [x] `docs/user-flow-guide_by_kimi.md` 已更新成功指标采集管道使用说明、staging 环境搭建指南、schema 同步校验流程
- [x] `docs/FULL-COVERAGE-MATRIX.md` 已补充 S13 上线 Gate 覆盖的测试项与指标映射
- [x] `docs/sandbox-simulation-report.md` 已补充 staging 回归在沙箱中的验证方法与数据注入示例

## 任务完成状态
- [x] U13 — 成功指标采集、Schema 一致性与 0→6 阶严格回归
