...
## 架构红线合规
- [ ] 新增 `scripts/lib/heartbeat_handler.py` 独立模块，心跳处理未直接追加到 `orch_gateway.py`
- [ ] `scripts/lib/worker_session_sweeper.py` 增强后，sweeper 逻辑未直接追加到 `orch_gateway.py`
- [ ] `orch_gateway.py` 净增长 ≤ 50 行
- [ ] Sweeper 与 Gateway 主进程分离运行（验证 `ps` 或进程列表中存在独立 sweeper 进程/线程）
- [ ] `write_scope_validator.py` 被 S8 与 S9 复用，无代码重复

## 文档交付物
- [ ] `docs/sandbox-simulation-report.md` 已更新，包含心跳协议 schema 与 sweeper 调度说明
- [ ] `docs/user-flow-guide_by_kimi.md` 已更新，包含实时快照查询的交互说明与错误码释义
- [ ] `docs/sprints/prd-by-kimi-user-flow-strict/schema.md` 已补充 `heartbeat_message`、`sweep_result`、`write_set`、`merge_strategy_enum` 数据模型
- [ ] `docs/gateway-integration-architecture.md` 已更新，包含心跳→快照→sweeper 的数据流图
- [ ] `events.jsonl` 中存在 `heartbeat`、`sweep_result`、`parallel_write_conflict` 三类事件记录

## 任务完成状态
- [ ] U9 — 三阶心跳、实时快照与并行写集策略（所有 AC 断言通过）
