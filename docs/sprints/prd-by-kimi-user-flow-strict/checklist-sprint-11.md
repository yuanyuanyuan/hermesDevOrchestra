...
## 架构红线合规
- [x] 新增逻辑优先落入 `scripts/lib/gateway_evaluation.py`，`orch_gateway.py` 仅保留路由与编排
- [x] `orch_gateway.py` 行数增长不超过 50 行（超支需记录技术债务）
- [x] 评估引擎支持独立单元测试（mock artifacts），不依赖真实上游 Run
- [x] 文件态持久化采用原子写模式

## 文档交付物
- [x] `docs/user-flow-guide_by_kimi.md` 已更新五阶 8 维评分流程、mode 触发场景矩阵、notification_level 行为对照表
- [x] `config/debate/full/coverage-policy.json` 已写入 `pass_with_warnings` 默认触发条件配置
- [x] `docs/FULL-COVERAGE-MATRIX.md` 已补充五阶覆盖维度与测试映射

## 任务完成状态
- [x] U11 — 五阶八维评估、残余风险通知与 authority routing
...
[2026-06-03] 已合并到 main — PR #29 (`feat/sprint11`), merged 2026-06-03T03:21:58Z
