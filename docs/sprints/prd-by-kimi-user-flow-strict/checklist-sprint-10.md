...
## 架构红线合规
- [x] 新增逻辑优先落入 `scripts/lib/gateway_improvement.py`，`orch_gateway.py` 仅保留路由与编排
- [x] `orch_gateway.py` 行数增长不超过 50 行（超支需记录技术债务）
- [x] 所有自动推进路径附带阻塞条件、authority route 和回放验证路径
- [x] 文件态持久化采用原子写模式（先临时文件后重命名）

## 文档交付物
- [x] `docs/user-flow-guide_by_kimi.md` 已更新四阶 A-E 分类处理流程图
- [x] `docs/FULL-COVERAGE-MATRIX.md` 已补充四阶覆盖维度与测试映射

## 任务完成状态
- [x] U10 — 四阶改进分类、回归上限与争议裁决
...
[2026-06-03] Verified by Codex — all Sprint 10 local tests passed
[2026-06-03] 已合并到 main — PR #28 (`feat/sprint10`), merged 2026-06-03T02:54:15Z
