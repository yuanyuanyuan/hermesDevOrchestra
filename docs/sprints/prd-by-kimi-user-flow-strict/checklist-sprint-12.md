...
## 架构红线合规
- [x] 新增逻辑优先落入 `scripts/lib/gateway_closeout.py`，`orch_gateway.py` 仅保留路由与编排
- [x] `orch_gateway.py` 行数增长不超过 50 行（超支需记录技术债务）
- [x] Queue 持久化目录 `.hermes/evolution-queue/` 不在 `/tmp` 等易失路径
- [x] Queue 写入采用原子写模式（先临时文件后重命名）
- [x] protected target 审批引用缺失时 Gateway 必须拒绝 closeout

## 文档交付物
- [x] `docs/user-flow-guide_by_kimi.md` 已更新六阶审计输入清单、protected target 审批流程图、queue 持久化说明
- [x] `docs/FULL-COVERAGE-MATRIX.md` 已补充六阶覆盖维度与测试映射
- [x] `docs/sandbox-simulation-report.md` 已补充 closeout 完整性校验在沙箱中的验证方法

## 任务完成状态
- [x] U12 — 六阶审计沉淀与 protected target 审批
...
[2026-06-03] 已合并到 main — PR #30 (`feat/sprint12`), merged 2026-06-03T04:19:05Z
