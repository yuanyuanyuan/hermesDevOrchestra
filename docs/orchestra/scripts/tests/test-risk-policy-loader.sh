...
cp "$REPO_ROOT/docs/orchestra/config/risk-policy.yaml" "$HOME/.hermes-orchestra/risk-policy.yaml"
...
"$REPO_ROOT/docs/orchestra/scripts/bin/orch-risk-check" "echo hello" >"$SAFE_OUT"
safe_status=$?
...
set +e
"$REPO_ROOT/docs/orchestra/scripts/bin/orch-risk-check" "docker system prune -af" >"$L3_OUT"
l3_status=$?
set -e
...
set +e
"$REPO_ROOT/docs/orchestra/scripts/bin/orch-risk-check" "git push origin main --force" >"$L4_OUT"
l4_status=$?
set -e
...
set +e
"$REPO_ROOT/docs/orchestra/scripts/bin/orch-risk-check" --role reviewer --tool terminal "ls -la" >"$ROLE_OUT"
role_status=$?
set -e
